...
python3 -m http.server 4980 &

echo "$(date) - Watching directory: $WATCH_DIR for new files..." | tee -a "$LOG_FILE" | websocat ws://0.0.0.0:4981
echo "$(date) - Watching directory: $WATCH_DIR2 for new files..." | tee -a "$LOG_FILE2" | websocat ws://0.0.0.0:4981
echo "$(date) - Watching directory: $WATCH_DIR3 for new files..." | tee -a "$LOG_FILE3" | websocat ws://0.0.0.0:4981
  ...
  for FILE in "$WATCH_DIR"/*; do
    if [[ -f "$FILE" ]] && check_extension "$FILE"; then
      if is_file_complete "$FILE"; then
        echo "$(date) - File is complete: $FILE" | tee -a "$LOG_FILE" | websocat ws://0.0.0.0:4981

        filename=$(basename "$FILE")
        in_basename="${filename%.*}"
        out_basename="${in_basename}_encoded_${PRESET_NAME}"

        echo "$(date) - Processing file: $FILE" | tee -a "$LOG_FILE" | websocat ws://0.0.0.0:4981
        ...
        if [ -n "$AUDIO_STREAM" ]; then
          /app/venv/bin/ffmpeg-normalize "$FILE" -o "$out_path" -ext "$OUT_EXTENSION" -e="$VIDEO_OPTIONS" -c:a "$AUDIO_CODEC" -b:a "$AUDIO_BITRATE" -nt "$STANDARD" -t "$TARGET_LOUDNESS" --dual-mono -ar "$SAMPLE_RATE" -v | tee -a "$LOG_FILE" | websocat ws://0.0.0.0:4981
        else
          # If no audio stream, just encode the video stream
          ffmpeg -i "$FILE" -c:v "$VIDEO_ENCODER" -b:v "$VIDEO_BITRATE" -r "$FRAME_RATE" -s "$RESOLUTION" -c:a copy "$out_path.$OUT_EXTENSION" -loglevel verbose | tee -a "$LOG_FILE" | websocat ws://0.0.0.0:4981
        fi

        echo "$(date) - Finished processing: $FILE" | tee -a "$LOG_FILE" | websocat ws://0.0.0.0:4981

        # move file
        mkdir -p "$WATCH_DIR/_original"
        mv "$FILE" "$WATCH_DIR/_original/"
      else
        echo "$(date) - File is still being written: $FILE" | tee -a "$LOG_FILE" | websocat ws://0.0.0.0:4981
        continue # Skip this file and check again later
      fi
    else
      if [[ -f "$FILE" ]]; then
        echo "$(date) - Not supported input format: $FILE" | tee -a "$LOG_FILE" | websocat ws://0.0.0.0:4981
        # move file
        mkdir -p "$WATCH_DIR/_original"
        mv "$FILE" "$WATCH_DIR/_original/"
      fi
  ...
  for FILE in "$WATCH_DIR2"/*; do
    if [[ -f "$FILE" ]] && check_extension "$FILE"; then
      if is_file_complete "$FILE"; then
        ...
        echo "$(date) - Processing file: $FILE" | tee -a "$LOG_FILE2" | websocat ws://0.0.0.0:4981
        #echo "Input file basename: $in_basename"
        #echo "Output file basename: $out_basename"
        ...
        if [ -n "$AUDIO_STREAM" ]; then
          ffmpeg -i "$FILE" -an -c:v "$VIDEO_ENCODER2" -b:v "$VIDEO_BITRATE2" -r "$FRAME_RATE2" -s "$RESOLUTION2" "$out_path.$OUT_EXTENSION2" -loglevel verbose | tee -a "$LOG_FILE2" | websocat ws://0.0.0.0:4981
        else
          # If no audio stream, just encode the video stream
          ffmpeg -i "$FILE" -c:v "$VIDEO_ENCODER2" -b:v "$VIDEO_BITRATE2" -r "$FRAME_RATE2" -s "$RESOLUTION2" -c:a copy "$out_path.$OUT_EXTENSION2" -loglevel verbose | tee -a "$LOG_FILE2" | websocat ws://0.0.0.0:4981
        fi

        echo "$(date) - Finished processing: $FILE" | tee -a "$LOG_FILE2" | websocat ws://0.0.0.0:4981
  ...
  for FILE in "$WATCH_DIR3"/*; do
    if [[ -f "$FILE" ]] && check_extension "$FILE"; then
      if is_file_complete "$FILE"; then
        ...
        echo "$(date) - Processing file: $FILE" | tee -a "$LOG_FILE3" | websocat ws://0.0.0.0:4981
        #echo "Input file basename: $in_basename"
        #echo "Output file basename: $out_basename"
        ...
        if [ -n "$AUDIO_STREAM" ]; then
          /app/venv/bin/ffmpeg-normalize "$FILE" -o "$out_path" -ext "$OUT_EXTENSION3" -vn -c:a "$AUDIO_CODEC3" -b:a "$AUDIO_BITRATE3" -nt "$STANDARD3" -t "$TARGET_LOUDNESS3" --dual-mono -ar "$SAMPLE_RATE3" -v | tee -a "$LOG_FILE3" | websocat ws://0.0.0.0:4981
        else
          # If no audio stream, log it and move file
          echo "No audio stream found, can't output normalized audio file." | tee -a "$LOG_FILE3" | websocat ws://0.0.0.0:4981
        fi

        echo "$(date) - Finished processing: $FILE" | tee -a "$LOG_FILE3" | websocat ws://0.0.0.0:4981
