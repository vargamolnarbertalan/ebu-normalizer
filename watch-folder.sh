...
echo "$(date) - Watching directory: $WATCH_DIR2 for new files..." | tee -a "$LOG_FILE" #| #websocat -1 ws://localhost:443/ws
echo "$(date) - Watching directory: $WATCH_DIR3 for new files..." | tee -a "$LOG_FILE" #| #websocat -1 ws://localhost:443/ws
  ...
  for FILE in "$WATCH_DIR"/*; do
    if [[ -f "$FILE" ]] && check_extension "$FILE"; then
      if is_file_complete "$FILE"; then
        ...
        echo "$(date) - Processing file: $FILE via $PRESET_NAME preset" | tee -a "$LOG_FILE" #| #websocat -1 ws://localhost:443/ws
        ...
        if [ -n "$AUDIO_STREAM" ]; then
          ...
          ffmpeg -i "$FILE" -c:v "$VIDEO_ENCODER" -b:v "$VIDEO_BITRATE" -r "$FRAME_RATE" -s "$RESOLUTION" -c:a copy "$out_path" -loglevel verbose | tee -a "$LOG_FILE" #| #websocat -1 ws://localhost:443/ws
        fi

        echo "$(date) - Finished processing: $out_path" | tee -a "$LOG_FILE" #| #websocat -1 ws://localhost:443/ws
  ...
  for FILE in "$WATCH_DIR2"/*; do
    if [[ -f "$FILE" ]] && check_extension "$FILE"; then
      if is_file_complete "$FILE"; then
        ...
        echo "$(date) - Processing file: $FILE via $PRESET_NAME2 preset" | tee -a "$LOG_FILE" #| #websocat -1 ws://localhost:443/ws
        #echo "Input file basename: $in_basename"
        #echo "Output file basename: $out_basename"
        ...
        if [ -n "$AUDIO_STREAM" ]; then
          ffmpeg -i "$FILE" -an -c:v "$VIDEO_ENCODER2" -b:v "$VIDEO_BITRATE2" -r "$FRAME_RATE2" -s "$RESOLUTION2" "$out_path" -loglevel verbose | tee -a "$LOG_FILE" #| #websocat -1 ws://localhost:443/ws
        else
          # If no audio stream, just encode the video stream
          ffmpeg -i "$FILE" -c:v "$VIDEO_ENCODER2" -b:v "$VIDEO_BITRATE2" -r "$FRAME_RATE2" -s "$RESOLUTION2" -c:a copy "$out_path" -loglevel verbose | tee -a "$LOG_FILE" #| #websocat -1 ws://localhost:443/ws
        fi
  ...
  for FILE in "$WATCH_DIR3"/*; do
    if [[ -f "$FILE" ]] && check_extension "$FILE"; then
      if is_file_complete "$FILE"; then
        ...
        echo "$(date) - Processing file: $FILE via $PRESET_NAME3 preset" | tee -a "$LOG_FILE" #| #websocat -1 ws://localhost:443/ws
        #echo "Input file basename: $in_basename"
        #echo "Output file basename: $out_basename"
        ...
        if [ -n "$AUDIO_STREAM" ]; then
          /app/venv/bin/ffmpeg-normalize "$FILE" -o "$out_path" -ext "$OUT_EXTENSION3" -vn -c:a "$AUDIO_CODEC3" -b:a "$AUDIO_BITRATE3" -nt "$STANDARD3" -t "$TARGET_LOUDNESS3" --dual-mono -ar "$SAMPLE_RATE3" -v | tee -a "$LOG_FILE" #| #websocat -1 ws://localhost:443/ws
        else
          # If no audio stream, log it and move file
          echo "No audio stream found, can't output normalized audio file." | tee -a "$LOG_FILE" #| #websocat -1 ws://localhost:443/ws
        fi
